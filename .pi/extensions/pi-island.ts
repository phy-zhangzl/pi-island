...
export default function (pi: ExtensionAPI) {
  ...
  const SEND_TIMEOUT_MS = 150;
  const MESSAGE_UPDATE_THROTTLE_MS = 400;
  ...
  async function send(payload: PiEventPayload) {
    ...
    try {
      ...
      // App may not be running or may be slow; never interrupt pi.
    } finally {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    hadError = false;
    getSessionName(ctx);
    emitState(ctx, "idle", "Ready");
  });

  pi.on("session_shutdown", async (_event, ctx) => {
    hadError = false;
    emitState(ctx, "idle", "Session ended");
  });

  pi.on("agent_start", async (_event, ctx) => {
    hadError = false;
    lastThinkingEmitAt = 0;
    emitState(ctx, "thinking", "Processing request");
  });

  pi.on("message_update", async (_event, ctx) => {
    if (hadError) return;
    if (currentState === "reading" || currentState === "running" || currentState === "patching") return;

    const now = Date.now();
    if (now - lastThinkingEmitAt < MESSAGE_UPDATE_THROTTLE_MS) return;
  ...
  pi.on("message_end", async (_event, ctx) => {
    ...
    emitState(ctx, currentState, currentDetail);
  });
  ...
  pi.on("tool_execution_start", async (event: any, ctx) => {
    if (event.toolName === "read") {
      emitState(ctx, "reading", readDetail(event.args));
    } else if (event.toolName === "bash") {
      emitState(ctx, "running", bashDetail(event.args));
    } else if (event.toolName === "edit" || event.toolName === "write") {
      emitState(ctx, "patching", patchDetail(event.args));
    }
  });
  ...
  pi.on("tool_result", async (event: any, ctx) => {
    if (event.isError) {
      hadError = true;
      emitState(ctx, "error", "Something went wrong");
    }
  });

  pi.on("agent_end", async (_event, ctx) => {
    if (hadError) return;
    emitState(ctx, "done", "Task completed");
  });
}
